...
pub struct PoseidonSpongeVar<F: PrimeField> {
    ...
    // Sponge State
    /// the sponge's state
    pub state: Vec<FpVar<F::BasePrimeField>>,
    /// the mode
...
///
/// [cos]: https://eprint.iacr.org/2019/1076
pub struct PoseidonSpongeVar<F: Field> {    // zeroknight : PrimeField -> Field
    /// constraint system
    pub cs: ConstraintSystemRef<F>,
    ...
    // Sponge State
    /// the sponge's state
    pub state: Vec<FpVar<F::BasePrimeField>>,   // zeroknight : F -> F::BasePrimeField
    //pub state: Vec<FieldVar<F>>,
    /// the mode
    mode: PoseidonSpongeMode,
}

impl<F: Field> PoseidonSpongeVar<F> {           // zeroknight : PrimeField -> Field, FpVar<F> -> FpVar<F::BasePrimeField>
    #[tracing::instrument(target = "r1cs", skip(self))]
    fn apply_s_box(
        &self,
        state: &mut [FpVar<F::BasePrimeField>],
        is_full_round: bool,
    ) -> Result<(), SynthesisError> {
        // Full rounds apply the S Box (x^alpha) to every element of state
        if is_full_round {
    ...
    #[tracing::instrument(target = "r1cs", skip(self))]
    fn apply_ark(&self, state: &mut [FpVar<F::BasePrimeField>], round_number: usize) -> Result<(), SynthesisError> {
        for (i, state_elem) in state.iter_mut().enumerate() {
            *state_elem += self.ark[round_number][i];
        }
        Ok(())
    ...
    #[tracing::instrument(target = "r1cs", skip(self))]
    fn apply_mds(&self, state: &mut [FpVar<F::BasePrimeField>]) -> Result<(), SynthesisError> {
        let mut new_state = Vec::new();
        let zero = FpVar::<F::BasePrimeField>::zero();
        for i in 0..state.len() {
            let mut cur = zero.clone();
            for (j, state_elem) in state.iter().enumerate() {
                let term = state_elem * self.mds[i][j];
    ...
    fn absorb_internal(
        &mut self,
        mut rate_start_index: usize,
        elements: &[FpVar<F::BasePrimeField>],
    ) -> Result<(), SynthesisError> {
        let mut remaining_elements = elements;
        loop {
            // if we can finish in this call
    ...
    fn squeeze_internal(
        &mut self,
        mut rate_start_index: usize,
        output: &mut [FpVar<F::BasePrimeField>],
    ) -> Result<(), SynthesisError> {
        let mut remaining_output = output;
        loop {
            // if we can finish in this call
